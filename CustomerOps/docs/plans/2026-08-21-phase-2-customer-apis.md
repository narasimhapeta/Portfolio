...
- Create: `src/CustomerPortal.Infrastructure/Persistence/CustomerPortalDbContextFactory.cs`
...
- [ ] **Step 7: Add a design-time factory, then generate the initial migration**

`dotnet ef migrations add --startup-project src/CustomerPortal.Api` needs to resolve `DbContextOptions<CustomerPortalDbContext>` from the Api project's DI container — but `Program.cs` doesn't register `CustomerPortalDbContext` until Task 4. Without a design-time factory, this step fails with `Unable to resolve service for type 'DbContextOptions...'`. A small `IDesignTimeDbContextFactory` sidesteps this by building `DbContextOptions` directly, independent of `Program.cs` — which also means migration generation won't break later as Program.cs picks up Redis/Service Bus dependencies in later phases.
...
Captured from bugs hit while executing this plan and from the CLAUDE.md §46 interview checkpoint at the end of the phase. Going forward, every plan gets this section filled in after its interview checkpoint — corrections and debugging findings recorded here so the next phase (and the next plan doc) starts from what was actually learned, not just what was designed.

### Debugging findings

- **EF Core design-time `DbContext` resolution.** `dotnet ef migrations add --startup-project <api>` needs the startup project's DI container to resolve `DbContextOptions<TContext>`. If `Program.cs` hasn't registered the `DbContext` yet — as was true here, since that wiring was Task 4 but migration generation was Task 3 — the command fails with `Unable to resolve service for type 'DbContextOptions...'`, produces no migration, and the failure doesn't surface until later as `Invalid object name 'Customers'` in tests that assume the schema exists. Fixed with an `IDesignTimeDbContextFactory<TContext>` in the Infrastructure project (`CustomerPortalDbContextFactory`), which builds `DbContextOptions` independently of `Program.cs`. This is also the more durable choice as `Program.cs`'s DI graph grows in later phases (Redis, Service Bus) — migration generation shouldn't depend on the whole app host booting successfully. Take-away for future plans: **when a plan generates an EF Core migration in an earlier task than the one that registers the `DbContext` in `Program.cs`, add the design-time factory in that same earlier task**, not as an afterthought.
- **Missing `using` for an extension method.** `services.RemoveAll<T>()` lives in `Microsoft.Extensions.DependencyInjection.Extensions`, not `Microsoft.Extensions.DependencyInjection` — `CustomerApiFactory.cs` only had the latter, giving `CS1061`. Easy to miss because both namespaces are commonly imported together elsewhere in ASP.NET Core code.

### Interview checkpoint Q&A

**Q1. Why does `ICustomerRepository` live in Application instead of Infrastructure, and what breaks if `CustomerService` referenced `CustomerRepository` directly?**
Answer given: Dependency Inversion — Infrastructure can change without Application changing.
Assessment: correct, and worth stating precisely — referencing `CustomerRepository` directly would give Application a compile-time dependency on Infrastructure (and transitively EF Core), inverting the Clean Architecture dependency rule (dependencies point inward). Concretely it would mean losing the fast `InMemoryCustomerRepository`-backed unit test suite from Task 2, since `CustomerService` tests would need a real or mocked `DbContext` instead.

**Q2. Two concurrent `DELETE` requests for the same customer id — what does each one actually get back?**
Answer given: first 200, second 404.
Assessment: corrected — the `Deactivate` action returns `204 No Content`, not 200; and **both** concurrent requests get 204. Because `DELETE` is a soft delete (`Customer.Deactivate()`), the row is never removed, so `GetByIdAsync` still finds it on the second call (already `Inactive`) and `Deactivate()` on an already-inactive customer is a no-op state-wise. This makes the endpoint naturally idempotent — a property a *hard* delete wouldn't have (the second call would legitimately 404 once the row was gone). Relevant for later phases: CLAUDE.md §23 idempotency will come back for the Service Bus operation-submission endpoint, where it won't be free like this.

**Q3. Why a design-time factory instead of just moving `AddDbContext` from Task 4 earlier into Task 3?**
Answer given: so the SQL connection config doesn't need to change every time to test locally, given Azure SQL/containerization is coming.
Assessment: corrected — the factory doesn't reduce connection-string duplication (it actually adds a second hardcoded copy, separate from `appsettings.Development.json`). The real reason is decoupling "can EF build a `DbContextOptions`" from "can the whole app host boot" — as `Program.cs` picks up Redis and Service Bus in later phases, requiring a full successful host boot just to add a migration would get increasingly fragile (e.g., migrations blocked because the Service Bus emulator isn't running, even though migrations only touch the `DbContext`).

**Q4. What bug class does testing against real SQL Server (Testcontainers) catch that a mocked/fake repository never could?**
Answer given: DB migration bugs.
Assessment: correct instinct, sharpened to a broader category — anything where the fake's behavior silently diverges from what SQL Server actually does. Concrete examples from this phase's own tests: `SearchAsync_MatchesByLastNameCaseInsensitive` passes against SQL Server because of its default case-insensitive collation, while `InMemoryCustomerRepository` only matches case-insensitively because it explicitly uses `StringComparison.OrdinalIgnoreCase` — a real divergence a mock wouldn't expose. Also: `HasMaxLength`/index constraints are only enforced by the real engine; and whether LINQ (`EF.Functions.Like`, `Skip`/`Take`, `OrderBy`) actually *translates* to valid T-SQL at all is untested by a fake.
